...
const reasons = [
  {
    ...
    desc: (
      <>
        AI 이미지 생성(Gemini, Flux)은 수 초~수십 초가 걸리는 작업입니다.
        <br />
        요청자가 완료를 기다리지 않고 <strong className="text-blue-300">바로 응답</strong>받을 수 있습니다.
      </>
    ),
  ...
  },
  {
    ...
    desc: (
      <>
        요청 서버(Producer)와 처리 서버(Consumer)가 <strong className="text-green-300">독립적으로 동작</strong>합니다.
        <br />
        각각 별도로 스케일링, 배포, 장애 대응이 가능합니다.
      </>
    ),
  ...
  },
  {
    ...
    desc: (
      <>
        워커가 죽어도 메시지가 큐에 남아있어 <strong className="text-yellow-300">재처리가 가능</strong>합니다.
        <br />
        ACK/NACK 메커니즘으로 메시지 손실을 방지합니다.
      </>
    ),
    details: ['처리 실패 시 실패응답', '서버 재시작해도 작업 유지'],
  },
  {
    num: '04',
    title: '부하 분산',
    ...
    desc: (
      <>
        여러 워커가 동시에 작업을 <strong className="text-purple-300">분산 처리</strong>할 수 있습니다.
        <br />
        prefetch_count로 워커당 처리량을 조절합니다.
      </>
    ),
    ...
    <SsgoiTransition id="/slide-5-1">
      <SlideContainer nextSlide="/slide-5-2">
        ...
        <SlideHeader
          ...
          title={
            <>
              왜 <span className="text-orange-400">RabbitMQ</span>를 사용할까?
            </>
          }
