...
export default function FlowOverviewSlide() {
  ...
  const handleStepClick = (link?: string) => (e: React.MouseEvent) => {
    if (link) {
      e.stopPropagation()
      navigate(link)
    }
    ...
    <SsgoiTransition id="/slide-2">
      <SlideContainer>
        ...
        <SlideHeader
          ...
          title={
            <>
              한눈에 보는 <span className="text-blue-400">전체 흐름</span>
            </>
          }
        />
        ...
        <div className="main-content justify-center">
          ...
          <div className="flex items-center justify-between mb-12 relative">
            {flowSteps.map((step, index) => (
              <div key={step.num} className="flex items-center">
                {/* Step */}
                <div className={`flex flex-col items-center w-[140px] relative z-[2] ${step.link ? 'cursor-pointer hover:scale-105 transition-transform' : ''}`} onClick={handleStepClick(step.link)}>
                  <div className="absolute -top-1 -left-1 w-6 h-6 bg-sky-400 text-slate-900 rounded-full flex items-center justify-center font-bold text-xs border-2 border-slate-900">{step.num}</div>
                  <div
                    className={`w-20 h-20 rounded-full flex items-center justify-center text-3xl mb-4 shadow-md ${
                      step.highlight ? 'bg-sky-400 text-slate-900 border-sky-200' : 'bg-slate-800 border-sky-400 text-sky-400'
                    } border-2`}
                    {...(step['data-hero-key'] ? { 'data-hero-key': step['data-hero-key'] } : {})}
                  >
                    <i className={`fas ${step.icon}`}></i>
                  </div>
                  <p className="text-base font-bold text-center text-white mb-1">{step.label}</p>
                  <p className="text-xs text-center text-slate-400 leading-snug whitespace-pre-line">{step.desc}</p>
                </div>
          ...
          <div className="flex gap-5 justify-center mt-5 bg-slate-800/50 rounded-2xl p-6 border border-slate-700">
            {metrics.map((metric, index) => (
              <div key={index} className="flex items-center bg-slate-800 py-4 px-6 rounded-xl border-l-4 border-l-sky-400 min-w-[250px]">
                <div className="text-2xl text-slate-500 mr-4">
                  <i className={`fas ${metric.icon}`}></i>
                </div>
                <div>
                  <h4 className="text-slate-400 text-xs uppercase tracking-wider mb-1">{metric.title}</h4>
